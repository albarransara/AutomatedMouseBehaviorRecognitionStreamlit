...
with mode[0]:
    ...
    st.markdown("In manual mode you can upload videos, with their corresponding DeepLabCut labeled csv, files and you will get the video with labels, as well as some stadistics.")
    ...
    uploaded_csvs_manual = st.file_uploader("Upload CSV files",type=["csv"],accept_multiple_files=True,key="manualcsv")
    uploaded_videos_manual = st.file_uploader("Upload Video files",type=["mp4"],accept_multiple_files=True,key="manualvideos")
    ...
    if len(uploaded_csvs_manual) > 0:
        ...
        for csv_name in tab_names_manual_manual:
            ...
            if corresponding_video_name not in video_names_manual or len(uploaded_csvs_manual) != len(uploaded_videos_manual):
...
with (mode[1]):
    ...
    st.markdown("In automatic mode you can upload videos with their corresponding unlabeled DeepLabCut csv files and you will automatically get them labeled.")
