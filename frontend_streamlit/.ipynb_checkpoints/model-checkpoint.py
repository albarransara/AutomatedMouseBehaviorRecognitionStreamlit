...
def predict_video(features, behaviours):
    ...
    for i in range(0, len(features), 300):
        ...
        if "grooming" in behaviours:
            ...
            np.append(grooming,model.predict(f2))
        elif "mid_rearing" in behaviours:
            # Load grooming model
            model = tf.keras.models.load_model('resnet_lstm_accuracy_mid_rearing.h5')
            mid_rearing += np.array(model.predict(f2))
        elif "wall_rearing" in behaviours:
            # Load grooming model
            model = tf.keras.models.load_model('resnet_lstm_accuracy_wall_rearing.h5')
            wall_rearing += np.array(model.predict(f2))

    # Generate dataframes with the results
    if "grooming" in behaviours:
        results_percentage['grooming'] = grooming[0][:]
    elif "mid_rearing" in behaviours:
        results_percentage['mid_rearing'] = mid_rearing[0][:]
    elif "wall_rearing" in behaviours:
        results_percentage['wall_rearing'] = wall_rearing[0][:]
    ...
    for i in range(0, len(features), 300):
        ...
        result = model_g.predict(f2)
        grooming += result.tolist()

        result = model_mr.predict(f2)
        mid_rearing += result.tolist()

        result = model_wm.predict(f2)
        wall_rearing += result.tolist()
